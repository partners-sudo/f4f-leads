        ...
        try:
            ...
            with httpx.Client(timeout=10.0, follow_redirects=True) as client:
                ...
                if response.status_code == 200:
                    ...
                    # If we found emails, we can stop checking other pages
                    if emails:
                        break
...
def find_emails(domain: str, company_name: Optional[str] = None, verify: bool = True) -> List[Tuple[str, float]]:
    """
    ...
    # Strategy 1: Scrape website for emails
    logger.info("   Strategy 1: Scraping website...")
    ...
    # Strategy 2: Generate candidates and verify
    logger.info("   Strategy 2: Generating email candidates...")
    ...
    if verify:
        for candidate in candidates[:50]:  # Limit to top 50 to avoid too many requests
            if candidate not in seen_emails:
                ...
                if verified_email and score > 0.3:  # Only include emails with decent score
                    email_scores.append((verified_email, score))
                    logger.info(f"   ✓ Verified candidate: {verified_email} (score: {score:.2f})")
    else:
    ...
    logger.info(f"   ✅ Found {len(email_scores)} verified emails")
